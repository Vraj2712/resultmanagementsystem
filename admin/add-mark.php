                ...
                <div class="row">
                 <div class="col-lg-12">
                     <div class="callout callout-danger" style='margin-top: 30px;'>
                        <p>Note: Create exam, class, section & subject before add mark</p>
                     </div>
        ...
        if($rowcount > 0){
         $db_result = $conn->fetchSingle();
            ...
            $sql = "UPDATE results SET name_of_student =:name, class =:class, exam =:exam, 
                        section =:section, subject =:subject, first_ca =:first_ca, second_ca=:second_ca, 
                        school_year =:exam_year, exam_score =:exam_score, 
                        current_school_session =:current_school_session WHERE id =:id";
            $conn->query($sql);
            $conn->bind(":name", $student_name[$index]);
            $conn->bind(":exam", $current_exam[$index]);
            $conn->bind(":class", $student_class[$index]);
       ...
       //try the second query for student positions
            ...
            if($rowcount2 > 0){
                ...
                try{
                    ...
                    $success_msg2 ="<script>
                         toastr['success']('Student\'s scores were updated successfully too.');
                         </script>";
                }catch (PDOException $err){
                    $error = $err->getMessage();
                    $success_msg2 = "<script>
                ...
                try{
                    $conn->execute();
                    $success_msg2 = "<script>
                         toastr['success']('Student\'s scores were added successfully too.');
                         </script>";
                }catch (PDOException $err){
                    $error = $err->getMessage();
                    $success_msg2 = "<script>
